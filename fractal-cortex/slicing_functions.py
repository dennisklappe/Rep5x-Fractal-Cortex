...
def write_5_axis_gcode(newFile, savedFileName, printSettings, startingPositions, directions, chunk_transform3DList, adhesionList, chunk_shellRingsListList, chunk_optimizedInternalInfills, chunk_optimizedSolidInfills):
    ...
    """ HEADER """
    openFile.write(";SLICER:       Fractal Cortex (Rep5x)\n")
    openFile.write(";FIRMWARE:     Marlin (Rep5x)\n")
    ...
    for key in chunk_transform3DList: # For each chunk
        ...
        if key != '0': # If it's not the initial chunk, reorient B and C axes
            newChunk = True
            # AMOVE_Degrees -> C axis (yaw), BMOVE_Degrees -> B axis (tilt)
            c_deg = round(AMOVE_Degrees[int(key)], 5)
            b_deg = round(BMOVE_Degrees[int(key)], 5)
            if c_deg != 0 or b_deg != 0:
                openFile.write("G0 F" + str(G0Z_FEEDRATE) + " Z" + str(round(nozzleHeight + 10.0, 5)) + " ;Lift tool tip for reorientation\n")
                openFile.write("G0 B" + str(b_deg) + " C" + str(c_deg) + " ;Reorient B/C axes (IK compensates)\n")
            else:
